...
public class GeneralUtil {
    ...
    public static String getPropertyNameFromGetSet(String methodName) {
        ...
        return StringUtil.lowerFirstLetter(methodName.substring(3));
    }
    ...
    public static String splitDBFieldName(String fieldName, String prefix, String stuffix) {
        ...
        for (int i = 0; i < array.length; i++) {
            if (i == 0) {
                ...
                builder.append(StringUtil.upperFirstLetter(array[i]));
            }
        }
        return builder.toString();
    }
    ...
    public static Map<String, Object> object2Map(Object object, boolean toDB) {
        ...
        ReflectionUtil.getFields(fieldList, object.getClass(), true);
        if (isEmptyCollection(fieldList)) {
            return null;
        }
        ...
        for (Field field : fieldList) {
            ...
            if (toDB) {
                map.put(splitCamelName(field.getName()), ReflectionUtil.getFieldValue(object, field.getName()));
            } else {
                map.put(field.getName(), ReflectionUtil.getFieldValue(object, field.getName()));
            }
        }
        return map;
    }
    ...
    public static Object[] object2Array(Object object, Set<String> filterFields) {
        ...
        for (Field field : fieldList) {
            if (isNotEmptyCollection(filterFields)) {
                if (filterFields.contains(field.getName())) {
                    continue;
                }
            }
            Object val = ReflectionUtil.getFieldValue(object, field.getName());
            objList.add(val);
        }
    ...
    public static Object map2Object(Map<String, Object> map, Class target) {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            if (!ReflectionUtil.hasThisField(target, propertyName)) {
                continue;
            }
            Object value = entry.getValue();
            if (value instanceof String) {
                if (StringUtil.isDate(value.toString())) {
                    value = StringUtil.string2Date(value.toString());
                }
            }
            ReflectionUtil.setFieldValue(object, propertyName, value);
        }
        return object;
    }
    ...
    public static Map<String, List<Object>> mapHorizontal2Vertical(List<Map<String, Object>> mapList) {
        ...
        for (Map<String, Object> map : mapList) {
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                ...
                if (isEmptyCollection(columnList)) {
    ...
    public static Object[][] appendParams(Object[][] params, Map<String, Object> whereParams) {
        ...
        for (Object[] param : params) {
            param = ArrayUtil.mergeArray(param, appendParam);
        }
        return params;
    }
    ...
    public static String getUpdateSql(Class targetClass, Map<String, Object> params, Set<String> filterFields, String tablePrefix, String tableStuf, String tableName) {
        ...
        List<Field> fieldList = new ArrayList<Field>();
        ReflectionUtil.getFields(fieldList, targetClass, true);
        if (isEmptyCollection(fieldList)) {
            return null;
        }
        int count = 0;
        int index = 0;
        for (Field field : fieldList) {
            String fieldName = splitCamelName(field.getName());
            index++;
            if (isEmptyString(fieldName) || fieldName.equals("serial_version_u_i_d") || fieldName.equals("serialVersionUID")) {
                continue;
            }
            if (isNotEmptyCollection(filterFields)) {
    ...
    public static String getDeleteByKeysSql(Class targetClass, String keyFieldName, int paramsCount, String tablePrefix, String tableStuf, String tableName) {
        ...
        String table = getTableName(targetClass, tablePrefix, tableStuf, tableName);
        if (isEmptyString(table) || paramsCount <= 0) {
            return null;
        }
        StringBuilder builder = new StringBuilder();
        builder.append("delete from ").append(table);
    ...
    public static String getDeleteSqlWithConditions(Class targetClass, Map<String, Object> params, String tablePrefix, String tableStuf, String tableName) {
        if (isEmptyMap(params) || null == targetClass) {
            return null;
        }
        String table = getTableName(targetClass, tablePrefix, tableStuf, tableName);
        if (isEmptyString(table)) {
    ...
    public static String getQuerySqlWithConditions(Class targetClass, Map<String, Object> params, List<String> displayFields, String tablePrefix, String tableStuf, String tableName) {
        ...
        if (isEmptyCollection(displayFields)) {
            builder.append("*");
        } else {
    ...
    public static String getTableName(Class targetClass, String tablePrefix, String tableStuf, String tableName) {
        if (null == targetClass && isEmptyString(tableName)) {
            return null;
        }
        String table = "";
        if (isEmptyString(tableName)) {
            table = splitCamelName(targetClass.getSimpleName(), tablePrefix, tableStuf);
        } else {
            table = tableName;
        }
        return table;
    ...
    public static String execCommand(String command) {
        ...
        try {
            ...
            try {
                // 主进程阻塞，直到当前进程执行完毕后，主进程才会继续向下执行(注意是进程，不是线程)
                process.waitFor();
